...
from services.pricing_ai_service import predict_pricing_adjustment
...
def generate_policy_quote(user):
    ...
    pricing_ai = predict_pricing_adjustment(zone_snapshot, earnings)

    base_premium = current_app.config["DEFAULT_WEEKLY_PREMIUM"]
    dynamic_premium = earnings * (0.018 + (risk_score * 0.024))
    trigger_loading = round(len(zone_snapshot["automated_triggers"]) * 0.75, 2)
    weekly_premium = round(
        max(
            base_premium - pricing_ai["safe_zone_discount"],
            base_premium
            + dynamic_premium
            + pricing_ai["predicted_premium_delta"]
            - pricing_ai["safe_zone_discount"]
            + trigger_loading,
        ),
        2,
    )
    ...
    pricing_breakdown = {
        ...
        "ml_pricing_delta": pricing_ai["predicted_premium_delta"],
        "safe_zone_discount": pricing_ai["safe_zone_discount"],
        ...
        "coverage_hours_boost": pricing_ai["coverage_hours_boost"],
        "pricing_model_metrics": pricing_ai["model_metrics"],
        "pricing_features": pricing_ai["feature_vector"],
        "zone_snapshot": zone_snapshot,
    }

    return {
        "plan_name": "RouteRelief Weekly Shield",
        "weekly_premium": weekly_premium,
        "coverage_amount": coverage_amount,
        "coverage_hours": pricing_ai["coverage_hours"],
...
def sync_policy(policy):
    ...
    while policy.auto_renew and policy.current_term_end < now:
        ...
        policy.coverage_hours = quote["coverage_hours"]
...
def purchase_policy(user):
    ...
    policy = Policy(
        ...
        coverage_hours=quote["coverage_hours"],
        risk_score=quote["risk_score"],
